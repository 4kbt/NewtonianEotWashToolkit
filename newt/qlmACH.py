...
def tetrahedron(L, rho, x, y, z):
    ...
    qlm[4, L] = q00*(3*x**4+x**2*y**2+3*y**4-4*(x**2+y**2)*z**2+8*z**2)*3/280
    q41 = z*(3*x**3-1j*x**2*y-2j*y**3+4j*y*z**2+x*(y**2-4*z**2))
    ...
    qlm[5, L] = q00*q50*np.sqrt(11)/4486
...
def pyramid(L, rho, h, x, y):
    ...
    qlm[5, L+4] = q00*(3*x**4-10*x**2*y**2+3*y**4)*np.sqrt(11/70)*3/1024
